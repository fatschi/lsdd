...
public class ReleaseYearDifference implements NegativeRule{
	...
	public boolean duplicateRuledOut(PactRecord record1, PactRecord record2) {
		String record1DiscRelease = record1.getField(MultiBlocking.DISC_RELEASED_FIELD, PactString.class).getValue().toLowerCase();
		String record2DiscRelease = record2.getField(MultiBlocking.DISC_RELEASED_FIELD, PactString.class).getValue().toLowerCase();
		try{
			if(!record1DiscRelease.isEmpty() && !record2DiscRelease.isEmpty() &&
					Math.abs(Integer.valueOf(record1DiscRelease)-Integer.valueOf(record2DiscRelease))>MAX_YEAR_DIFF){
				return true;
			}
		} 
